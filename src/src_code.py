...
circle = make_circles(n_samples = n_samples, factor = 0.2, noise = noise_circle, random_state= random_state)
...
vl = VotingClassifier(estimators=[('svc',svc),('knn',knn),('dt',dt),('rf',rf),('ada',ada)])

names = ["SVC", "KNN", "Decision Tree", "Random Forest", "AdaBoost", "VL"]
classifiers = [svc, knn, dt, rf, ada, vl]
...
for ds_cnt, ds in enumerate(datasets):
    # preprocess dataseti split into training and test part
    X, y = ds
    ...
    cm = plt.cm.RdBu
    cm_bright = ListedColormap(['navy', 'darkorange'])

    ax = plt.subplot(len(datasets), len(classifiers) + 1, i)

    if ds_cnt == 0:
        ax.set_title("Input(Raw) Data")
